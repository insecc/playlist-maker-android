...
class PlaylistsRepositoryImpl(
    ...
    override fun getAllPlaylists(): Flow<List<Playlist>> = playlistDao.getAll().map { playlistsWithTracks ->
    ...
    override fun getPlaylist(playlistId: Long): Flow<Playlist?> = playlistDao.getPlst(playlistId).map { pwt ->
        pwt?.let {
            Playlist(
                id = it.playlist.id,
    ...
    override suspend fun addNewPlaylist(name: String, description: String, coverImageUri: String?) = withContext(Dispatchers.IO) {
        playlistDao.insertPls(PlaylistEntity(name = name, description = description, coverImageUri = coverImageUri))
    }

    override suspend fun deletePlaylistById(id: Long) = withContext(Dispatchers.IO) {
        playlistDao.deleteCR_plst(id)
        playlistDao.deletePlst(PlaylistEntity(id = id, name = "", description = ""))
    }
}