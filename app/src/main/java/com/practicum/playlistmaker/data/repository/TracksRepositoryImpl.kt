...
import com.practicum.playlistmaker.data.network.NtClient
...
class TracksRepositoryImpl(
    private val ntClient: NtClient
) : TracksRepository {
    override fun searchTracks(expression: String): Flow<Resource<List<Track>>> = flow {
        val response = ntClient.doRequest(SearchRequestTrack(expression))
        when (response.resultCode) {
            -1 -> emit(Resource.Error("NETWORK_ERROR"))
            200 -> {
                with(response as SearchResponseTrack) {
